<?php
	...
	$kar=mysqli_query($conn, "SELECT * FROM anggota WHERE id=$id");
	$data=mysqli_fetch_assoc($kar);

	if(isset($_POST['submit']))
	{
		if(ubahpersonil($_POST) > 0)
		{
			echo "
					<script>
						alert('data berhasil di ubah');
						document.location.href = 'personilband.php';
					</script>
				";
		}
		else{
				...
				<div class="menu">
					...
					<ul>
						...
						<li><a href="inputpersonilband.php">
								<div class="menu-inputpersonilband"></div>
							...
							<form  method="POST" action="" >
								<input type="hidden" name="id" value="<?= $data["id"]; ?>">
							  	<div class="form-group">
							    	...
							    	<input name="namalengkap" type="text" class="form-control" placeholder="nama lengkap" value="<?= $data["namalengkap"] ;?>" 
							    	>
							  	</div>
							 	<div class="form-group">
							    	<label for="exampleInputNotelepon">Nomor Telepon</label>
							    	<input name="telepon" type="text" class="form-control"  placeholder="input nomor telepon" value="<?= $data["telepon"] ;?>">
							    </div>
							    <div class="form-group">
		                        	<label for="alamat">Alamat</label>
		                            <div>
		                            	<textarea type="text" cols="50" rows="4" name="alamat" class="form-control" id="alamat" placeholder="Alamat" ><?= $data["alamat"] ;?></textarea>
		                        	</div>
		                    	</div>
								...
								<div class="form-group">
                                	<label>Tanggal</label>
                                	<input type="text" class="datepicker form-control penyiram-tanggal" name="tanggallahir" placeholder="masukkan tanggal" value="<?= $data["tanggallahir"] ;?>"> 
                                </div>
